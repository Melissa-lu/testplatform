...
<n-space style="height:100%">
  ...
  <!-- <n-form-item :label="$t('function.searchbugid')" style="margin:15px 0 -10px" label-placement="left">
    <n-input v-model:value="selectedbibugid" @blur="bugidchange" :placeholder="$t('function.bugdemo')" clearable />
  </n-form-item> -->
  <n-form-item style="width: 100%;" >
    <n-input-group style="width: 150%;">
      <n-input-group-label >{{$t('function.searchbugid')}}</n-input-group-label>
      <n-input :style="{ width: '100%' }" v-model:value="selectedbibugid" clearable />
      <!-- <n-button type="primary" ghost @click="bugidchange" >
        {{$t('common.search')}}
      </n-button> -->
    </n-input-group>
  </n-form-item>
  <n-form-item  style="margin:26px 0px -25px -15px;min-width: 500px;" label-placement="left">
    <n-input-group style="width: 150%;">
      <n-input-group-label >{{$t('function.fliterttleson')}}</n-input-group-label>
  ...
  /> -->
  <!-- 页面加载的动画 -->
  <!-- <div v-show="ifshowloading" style="background-color: #fdfdfd;color: #fdfdfd;width: 100%;height: 100%;position: absolute;top: 136px;z-index: 999;">11 -->
...
import { NIcon,NTag, NButton,Na,useDialog,NDialogProvider,useMessage } from "naive-ui";
import { inject } from 'vue'
import { LogInOutline as LogInIcon } from "@vicons/ionicons5";
import service2 from '../../requests/axios2';
import biapartFliter from '../../assets/data/biapartFliter.json'
...
export default {
  ...
  setup() {
    ...
    const linkapart = {
      ...
      filter: true,
      filterOptionValues: [],
      filterOptions: biapartFliter.value,
    };
    ...
    // 初始、切换数据加载
    function query (page, pageSize,filterValues2 = [],filterValues3 = "",filterValues4 = "") {
      return new Promise((resolve) => {
        datalist = [] ;//清空再赋值，不然每次查询会累加
        // console.log("vbanben!!!!!!!",filterValues2)
        // 默认是最新解决版本-1
        // if(minorversionRef.value==null){

        service2.get('/ScenarioAnalysis/ByBiId/'+selectedbibugidRef.value+'/').then(res=>{
          if(res.data.code != 0){
            //错误处理
            ...
            // 先过滤
            let filteredData3 = [{id:selectedbibugidRef.value,children:[]}];
            let filteredData2 = [{id:selectedbibugidRef.value,children:[]}];
            ...
            // console.log("vbanben!!!!!!!2.2",filteredData2);
            if(filterValues3 != ""){
              // console.log("vbanben!!!!!!!2.2",);
              if(filterValues4 != ""){
                console.log("vbanben!!!!!!!2.3",filteredData2);
                if(filterValues4 == "1"){ //包含
                  filteredData3[i].children = filteredData2[i].children.filter((row) => {
                    console.log("vbanben!!!!!!!2.31",row);
            ...
            for(let j=0;j<filteredData3[i].children.length;j++){   
              ...
              if(j ==0 && filteredData3[i].children.length > 1){
                ...
                // 如果小于等于，就正常的
                if((arrindex % pageSize + filteredData3[i].children.length -1) <= pageSize){
                  comlennow = filteredData3[i].children.length; //需要合并的长度
                  ifflagindex=true;
                  // console.log("meifen   ",i,arrindex,data1[i].children.length,comlennow,);
                }
                // 如果大于，要处理第二页
                else if((arrindex % pageSize + filteredData3[i].children.length - 1) > pageSize ){
              ...
              let rowjson ={
                ...
                linkfunctionName:filteredData3[i].children[j].functionName,
                linkbugid:filteredData3[i].children[j].bugid,
                functionName:filteredData3[i].children[j].functionName,
    ...
    function querydetails (reportid = null,fatherid = null) {
      return new Promise((resolve) => {
        ...
        // 默认是最新解决版本-1
        // if(minorversionRef.value==null){
        service2.get('/ScenarioAnalysis/ByBiId/'+reportid+'/').then(res=>{
          if(res.data.code != 0){
            //错误处理
            // reject({ status: 500, msg: 'error' })
          }else{
            ...
            for(let i=0;i<data1.length;i++){
              ...
              if(data1[i].bugid!=reportid && data1[i].bugid!= fatherid){
                // 形成新的显示json
                let rowjson ={
                  label: data1[i].functionName+" : "+data1[i].bugid+" : "+data1[i].bugtitle ,
                  key: data1[i].bugid,
                  isLeaf: false,
                }
    ...
    return {
      ...
      selectedbibugid:selectedbibugidRef,
      ...
      bugidchange(value){
        console.log("222222222222222",value,selectedbibugidRef.value);
        ifshowloadingRef.value = true;
        if(selectedbibugidRef.value == ""){
          message.error( t("function.notputnull"), { duration: 3000 });
          ifshowloadingRef.value = false;
        }
        else if(selectedbibugidRef.value.indexOf("report-")!=-1 || selectedbibugidRef.value.indexOf("REPORT-")!=-1){
          message.error( t("function.startbierr"), { duration: 3000 });
          ifshowloadingRef.value = false;
        }
        else{
          query(
      ...
      handlePageChange (currentPage) {
        if (!loadingRef.value) {
          ...
          console.log("data000:" ,functionNameReactive);
          query(
            currentPage,
            paginationReactive.pageSize,
            linkapartReactive.filterOptionValues,
      ...
      handleUpdateFilter (filters) {
        ...
        //   console.log("3333333333",filters.linkapart);
        ifshowloadingRef.value = true;
        if(selectedbibugidRef.value == ""){
          message.error( t("function.notputnull"), { duration: 3000 });
          ifshowloadingRef.value = false;
          ...
          service2.get('/ScenarioAnalysis/byid/'+node.key+'/').then(res=>{
            ...
            for(let i=0;i<data1.length;i++){
              ...
              if(flag == false){
                  // 形成新的显示json
                  let rowjson ={
                    label: data1[i].bugid+" "+data1[i].bugtitle+" "+data1[i].functionName ,
                    key: data1[i].bugid,
                    isLeaf: false,
                  }
                  details.push(rowjson);
